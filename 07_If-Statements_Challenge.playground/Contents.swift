...
if theAnswerToEverything == 42 {
    print("Correct! The answer to life, the universe, and everything is 42!")
} else {
...
// Answer: 1 equal assigns a value to a variable, two equals compares
...
func coldWarmOrHot(guess: Int) -> String {
    ...
    if guess == difference {
        return "That's insane, Wayne! You've guessed the number exactly and burned yourself! 🔥 \(randomNumber)"
    } else if guess - difference < 10 {
        return "Great hypothesis, Papadopoulos! You're hot!!! 🥵 \(randomNumber)"
    } else if guess - difference < 25 {
        return "Oh my, Bill Nye! You're warmer! ☺️ \(randomNumber)"
    } else {
        return "No way Jose. You're cold. 🥶 \(randomNumber)"
    }
}
